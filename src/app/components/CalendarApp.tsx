...
const CalendarApp: React.FC<CalendarAppProps> = ({ projectId }) => {
  ...
  const createScheduleXEndDateTime = (
    date: Date,
...
>
  ...
  const generateCalendarEvents = (
    ...
    const selectedDayNumbers = project.days_selected.map(
      (day) => dayNameToNumber[day as keyof typeof dayNameToNumber]
    );
    ...
    while (currentDate <= endDate && videoIndex < videos.length) {
      ...
      if (selectedDayNumbers.includes(dayOfWeek)) {
        ...
        const startDateTime = createScheduleXDateTime(
          currentDate,
          project.time_slot_start
        );
        const endDateTime = createScheduleXEndDateTime(
          currentDate,
          project.time_slot_start,
        ...
        const isCompleted =
          project.streak && project.streak.length > videoIndex
        ...
        events.push({
          ...
          _customContent: {
            timeGrid: createCustomContent(
              video,
              videoIndex,
              isCompleted,
              videos.length
            ),
            dateGrid: createCustomContent(
              video,
              videoIndex,
              isCompleted,
              videos.length
            ),
            monthGrid: createCustomContent(
              video,
              videoIndex,
              isCompleted,
              videos.length
            ),
            monthAgenda: createCustomContent(
              video,
              videoIndex,
              isCompleted,
              videos.length
  ...
  useEffect(() => {
    const observer = new MutationObserver(() => {
      ...
      if (
        dropdown &&
        dropdown.parentElement?.className.includes("sx-header-view-select")
      ) {
  ...
  const toggleVideoMark = async (videoId: string, isChecked: boolean) => {
    try {
      ...
      const endpoint = isChecked
        ? "/api/mark-video-as-completed"
        : "/api/unmark-video-as-completed";
          ...
          const updatedEvents = prev.map((event) => {
            if (event.videoId === videoId) {
              const updatedEvent = {
                ...
                _customContent: {
                  timeGrid: createCustomContent(
                    { title: event.title, url: event.videoUrl },
                    videoIndex,
                    isChecked,
                    prev.length
                  ),
      ...
      console.error(
        `Error ${isChecked ? "marking" : "unmarking"} video:`,
        error
      );
    }
  };
  ...
  useEffect(() => {
    (window as any).handleVideoCompletion = (
      videoIndex: string,
      isChecked: boolean
    ) => {
  ...
  return (
    <div className="flex justify-center bg-white w-screen h-screen ml-2">
      ...
      <div className="flex justify-center gap-2 mt-20 right-0 font-semibold font-inter">
        {!shouldHideSidebar && <Sidebbar completionMap={completionMap} />}
        ...
        <div className="sx-calendar-container">
          ...
          <div className="mb-4 flex items-center justify-between calendar-controls">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={goToNext} className="nav-arrow" title="Next">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
            ...
            <div className="flex items-center gap-3">
              <label htmlFor="view-select" className="font-medium">
                View:
              </label>
              <select
                id="view-select"
